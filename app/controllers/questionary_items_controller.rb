class QuestionaryItemsController < ApplicationController
  # before_action :set_questionary, only: [ :new, :create]
...
def new
  ...
  def create
    ...
    # @questionary_item.questionary_id=@current_questionary
    respond_to do |format|
      if @questionary_item.save
        format.html { redirect_to '/questionary_choices/new/' + @questionary_item.id.to_s, num: 0 }
  ...
  #   @current_questionary = Questionary.find(params[:id])
  # end
#   def set_questionary
#     @questionary = Questionary.find params[:id]
# end
